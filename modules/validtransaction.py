...
class Trx(QMainWindow):
    def __init__(self, activeuser):
        ...
        self.setter.clicked.connect(self.knowdate)
        self.startsetter.clicked.connect(self.knowstartdate)
        self.endsetter.clicked.connect(self.knowenddate)


        self.savebutton.clicked.connect(self.knowinguser)
    ...
    def save_transaction(self):
        try:
            ...
            ''', (
                ...
                self.savedate, self.startdate, self.enddate
            ))
            self.conn.commit()
    ...
    # FUNGSI TUNJUKIN THE DATES (Kalo tombol dipencet)
    def knowdate(self):
        try:
            inputdate = self.calendarWidget.selectedDate()
            self.savedate = inputdate.toString("yyyy-MM-dd")
            self.tanggal.setText(self.savedate)
            
        except ValueError:
                QMessageBox.warning(self, "Error", "Input Tanggal gagal!")
                return
        
    def knowstartdate(self):
        try:
            inputstartdate = self.startcal.selectedDate()
            self.startdate = inputstartdate.toString("yyyy-MM-dd")
            self.startd.setText(self.startdate)
            
        except ValueError:
                QMessageBox.warning(self, "Error", "Input Tanggal gagal!")
                return
        
    def knowenddate(self):
        try:
            inputenddate = self.endcal.selectedDate()
            self.enddate = inputenddate.toString("yyyy-MM-dd")
            self.endd.setText(self.enddate)
    ...
    # FUNGSI UTAMA UNTUK INPUT (Check dan simpan aja)
    def knowinguser(self):
        try:
            ...
            self.category = self.kategori.currentText()
            ...
            if not all([self.type, self.nilai, self.savedate, self.category]):
